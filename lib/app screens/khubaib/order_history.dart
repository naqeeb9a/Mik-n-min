...
import 'package:mik_and_min/app%20screens/khubaib/track_order.dart';
...
errorHandlingOrderHistory(context, AsyncSnapshot snapshot1) {
  if (snapshot1.connectionState == ConnectionState.done) {
    if (snapshot1.data == false || snapshot1.data == "Server Error") {
      ...
      return Center(
          child: text(
              context, "No Order History", 0.04, CustomColors.customBlack));
    }
    else if (globalAccessToken.isEmpty) {
      return Center(
          child: text(context, "First You have to Login", 0.04,
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              text(context, "${index + 1}. Order #MIKNMIN5460${index + 1}",
                  0.03, CustomColors.customGrey),
              const SizedBox(),
              index == 2
                  ? text(context, "Confirmed", 0.03, CustomColors.customGrey)
                  : index == 4
                      ? trackButton(context, () {
                          CustomRoutes().push(
                            context,
                            TrackOrder(text1: "Order #MIKNMIN5460${index + 1}"),
                          );
                        })
                      : index == 5
                          ? trackButton(context, () {
                              CustomRoutes().push(
                                context,
                                TrackOrder(
                                    text1: "Order #MIKNMIN5460${index + 1}"),
                              );
                            })
        ...
        Divider(
          thickness: CustomSizes().dynamicWidth(context, 0.0015),
        ),
      ],
    ),
  );
...
Widget trackButton(context, function) {
  return InkWell(
    onTap: function,
    child: Container(
      padding: EdgeInsets.symmetric(
        vertical: CustomSizes().dynamicHeight(context, 0.001),
